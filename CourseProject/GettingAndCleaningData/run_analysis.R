...
download.file(url, destfile="./input/UCI HAR Dataset.zip")
unzip("./input/UCI HAR Dataset.zip", exdir="./input")

# reading the features and create a filter for the mean and std variables.
features <- read.table("input/UCI HAR Dataset/features.txt", quote="\"")
mean_and_std_filter <- grepl("*-mean\\(\\)*", features[[2]]) | grepl("*-std\\(\\)*", features[[2]])
...
# give the activities speaking labels.
